...
impl MunsellMatt {
    ...
    pub fn new(i:usize) -> Self {
        ...
        let data:[f64;MATT_N] = MUNSELL_MATT_DATA[i*MATT_N..(i+1)*MATT_N].iter().map(|v| *v as f64).collect::<Vec<f64>>().try_into().unwrap();
...
pub static MM_KEY_MAP: LazyLock<BTreeMap<&str, usize>> = LazyLock::new(||{
    ...
    MUNSELL_MATT_KEYS.iter().enumerate().for_each(|(value, &key)|{
