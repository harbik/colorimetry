...
fn ss_parse(input: &str, source_file: Option<&str>) -> Result<SpectrumFile> {
    ...
    for line in input.lines() {
        ...
        if in_data_format {
            for field in trimmed.split_whitespace() {
                format_fields.push(field.to_uppercase());
            }
            continue;
        }
        if in_data {
            ss_tokenize_into(trimmed, &mut data_tokens);
            continue;
        }

        // Header line: KEYWORD<tab>value
        if let Some((key, value)) = ss_split_kv(trimmed) {
            match key.as_str() {
                ...
                "INSTRUMENT_SERIAL" => meta.instrument_serial = ss_str(&value),
                ...
                "MODEL_NUM" => meta.model_num = ss_str(&value),
                "PROD_DATE" => meta.prod_date = ss_str(&value),
                "SERIAL" => meta.serial = ss_str(&value),
                "NMEASURE" => meta.nmeasure = value.trim().parse::<u32>().ok(),
    ...
    if format_fields.is_empty() {
        return Err(SpectrumFileError::SchemaValidation(
            "SpectraShop: no BEGIN_DATA_FORMAT/END_DATA_FORMAT block found".into(),
        ));
    }
    if data_tokens.is_empty() {
        return Err(SpectrumFileError::SchemaValidation(
            "SpectraShop: no BEGIN_DATA/END_DATA block found or block is empty".into(),
        ));
    }

    let n_fields = format_fields.len();
    if data_tokens.len() % n_fields != 0 {
        return Err(SpectrumFileError::SchemaValidation(format!(
            "SpectraShop: data token count ({}) is not a multiple of field count ({})",
            data_tokens.len(),
            n_fields
        )));
    }

    let n_records = data_tokens.len() / n_fields;
    let mut records: Vec<SpectrumRecord> = Vec::with_capacity(n_records);
    for r in 0..n_records {
        let row = &data_tokens[r * n_fields..(r + 1) * n_fields];
        records.push(ss_build_record(&format_fields, row, &meta, r, source_file)?);
    }
    ...
    if records.len() == 1 {
        Ok(SpectrumFile::Single {
            schema_version,
            spectrum: records.into_iter().next().unwrap(),
        })
    } else {
...
// Tokenize a SpectraShop data line into fields, respecting double-quoted strings.
...
fn ss_build_record(
    ...
    while i < fields.len() {
        ...
        match field {
            "SAMPLE_ID" => {
                sample_id = ss_str(val);
                i += 1;
            }
            "SAMPLE_NAME" => {
                sample_name = ss_str(val);
                i += 1;
            }
            "SPECTRAL_NM" => {
    ...
    let id = sample_id.clone().unwrap_or_else(|| format!("{}", idx + 1));
    let date = meta
        .created
        .as_deref()
